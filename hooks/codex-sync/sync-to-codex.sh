...
MCP_SYNCER="$SCRIPT_DIR/sync-mcp-to-codex.py"
...
if [ -f "$MCP_SYNCER" ]; then
  if python3 "$MCP_SYNCER" >/dev/null 2>>"$LOG"; then
    log "SYNCED[mcp]: project .codex/config.toml refreshed"
  else
    log "ERROR: MCP sync failed via $MCP_SYNCER"
  fi
fi
