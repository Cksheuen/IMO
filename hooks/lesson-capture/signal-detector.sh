...
# Prefer the per-session transcript when Claude Code provides it.
# Fall back to history.jsonl only when transcript_path is unavailable.
[ -z "$TRANSCRIPT_PATH" ] && TRANSCRIPT_PATH="$HOME/.claude/history.jsonl"
