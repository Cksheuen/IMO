...
当前已经有 `promotion-scan.py` 在**当前仓库开发态的项目级 `.claude/settings.json`** 中挂到 `Stop` / `SubagentStop` 上执行轻量扫描，但它目前只负责发现候选 note。
...
补充要求：如果未来在别的作用域复用这条桥，必须先验证真实挂载位置与触发事件；未挂载前，这份文档只能表示“桥接设计存在”，不能表示“运行链已接通”。
