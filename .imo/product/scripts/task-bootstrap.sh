...
resolve_claude_home() {
  ...
  while [ "$candidate" != "/" ]; do
    if [ "${candidate##*/}" = ".claude" ] && [ -d "$candidate/hooks" ]; then
      printf '%s\n' "$candidate"
      return
    fi
...
IMPLEMENTATION_PLAN_FILE="$TASK_DIR/implementation-plan.md"
...
- Materialize an explicit implementation plan artifact for orchestrated work
...
- An explicit implementation plan artifact exists before orchestration starts implementation
...
if [ ! -f "$IMPLEMENTATION_PLAN_FILE" ]; then
  cat <<EOF > "$IMPLEMENTATION_PLAN_FILE"
# Implementation Plan
...
- Update this file before dispatching implementer work

## Proposed Workstreams

| # | Workstream | Goal | File Ownership | Dependencies | Acceptance Criteria |
|---|------------|------|----------------|--------------|---------------------|
| 1 | Context and planning | Refine requirements and execution strategy | (assign before implementation) | None | Scope, constraints, and verification approach are explicit |
| 2 | Implementation | Execute approved changes | (assign before implementation) | Approved plan | Code changes map cleanly to the approved plan |
| 3 | Verification | Validate behavior and capture evidence | (assign before verification) | Implementation complete | Tests/review evidence are recorded before closeout |

## Execution Strategy

- Decide whether work should run in parallel or serial after file ownership is clear
- Lock file ownership before starting implementer subagents
- Record blockers, assumptions, and plan changes here before implementation begins

## Open Questions

- List any blocking questions that must be resolved before implementation starts

## Verification Plan

- Define the concrete checks to run after implementation
- Record expected evidence paths and reviewer steps
EOF
fi
...
- Implementation plan artifact: $IMPLEMENTATION_PLAN_FILE
EOF
fi
...
- Initial PRD / context / status / implementation plan / feature list seeded

## Next Step

- Complete scale assessment
- Refine acceptance criteria if the task scope becomes clearer
- Finalize implementation-plan.md before dispatching implementation work
